...
from .strategy import backtest_rsi_strategy, backtest_iron_condor_strategy
from .screener import select_test_stocks, select_iron_condor_stocks
...
def run_iron_condor_backtest(tickers, start_date, end_date, strategy_params=None, output_dir="backtest_results"):
    """
    Run a backtest of the iron condor strategy on a list of tickers.
    ...
    - Dictionary with backtest results
    """
    # Default iron condor strategy parameters
    default_params = {
        'portfolio_size': 1000000,
        'risk_per_trade': 0.02,
    ...
    print(f"Running iron condor backtest on {len(tickers)} tickers from {start_date} to {end_date}")
    print(f"Strategy parameters: {default_params}")
    ...
    for ticker in tickers:
        print(f"Backtesting {ticker}...")
        result, portfolio_df = backtest_iron_condor_strategy(
            ticker, 
            start_date, 
            end_date, 
            **default_params
    ...
    winning_trades = [t for t in all_trades if t['P/L'] is not None and t['P/L'] > 0]
    losing_trades = [t for t in all_trades if t['P/L'] is not None and t['P/L'] <= 0]
    ...
    total_premium = sum(t['Net Credit'] for t in all_trades if 'Net Credit' in t)
    ...
    performance_summary = pd.DataFrame([{
        ...
        'Profit Factor': r['Profit Factor'],
        'Trades': r['Number of Trades'],
        'Avg IV Entry (%)': r.get('Average IV at Entry (%)', 0),
        'Premium Collected ($)': r.get('Total Premium Collected ($)', 0)
    } for r in all_results])
    
    # Save backtest results
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    results_filename = os.path.join(output_dir, f"iron_condor_backtest_{timestamp}.pkl")
    summary_filename = os.path.join(output_dir, f"iron_condor_summary_{timestamp}.csv")
    ...
    print(f"Iron condor backtest results saved to {results_filename}")
    print(f"Performance summary saved to {summary_filename}")
...
def run_iron_condor_strategy_test(n_stocks=20, start_year=2020, 
                                 days_to_expiration=30, target_profit=50, max_loss=100, 
                                 wing_width=5, body_width=10, min_iv_rank=70,
                                 output_dir="backtest_results"):
    """
    Run a complete iron condor strategy test with the specified parameters
    
    Parameters:
    - n_stocks: Number of stocks to test
    - start_year: Start year for the backtest
    - days_to_expiration: Target days to expiration for options
    - target_profit: Target profit percentage of max profit
    - max_loss: Maximum loss percentage of max loss
    - wing_width: Width of condor wings in dollars
    - body_width: Width between short strikes in dollars
    - min_iv_rank: Minimum IV rank for entries
    - output_dir: Directory to save results
    
    Returns:
    - Dictionary with backtest results
    """
    # Select test stocks optimized for iron condor strategy
    test_tickers = select_iron_condor_stocks(n_stocks=n_stocks, min_iv_rank=min_iv_rank)
    
    if not test_tickers:
        print("No stocks selected for iron condor backtesting.")
        return None
    
    print(f"Selected {len(test_tickers)} stocks for iron condor backtesting.")
    ...
    return run_iron_condor_backtest(test_tickers, start_date, end_date, strategy_params, output_dir)